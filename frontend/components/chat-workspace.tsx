...
export function ChatWorkspace() {
  ...
  return <>
    <header className="topbar"><span className="chat-title">{currentThread?.title ?? "New chat"}</span><div className="model-picker">
      <span className={`status-dot${modelsError ? " offline" : ""}`} title={modelsError ? "Could not reach the model server" : "Model server connected"} />
    ...
    <div className="workspace">
      <aside className="workspace-sidebar"><button className="new-chat" onClick={() => setCurrentThreadId(null)}>＋ <span>New chat</span></button><div className="section-label">Recent</div>
        <div className="recent-list">{threads.map((thread) => <button key={thread.id} className={`recent-item${thread.id === currentThreadId ? " active" : ""}`} onClick={() => setCurrentThreadId(thread.id)}>{thread.title}</button>)}</div>
      </aside>
      <section className="chat-panel"><div className="conversation"><div className="thread">
        ...
        {isSending && <div className="typing" aria-label="Thinking"><span /><span /><span /></div>}
      </div></div>
      <form className="composer" onSubmit={submit}><div className="composer-inner"><input className="query-input" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Ask a question…" autoComplete="off" /><button className="send-btn" disabled={isSending} aria-label="Send">→</button></div></form>
      </section>
    </div>
  </>;
}
