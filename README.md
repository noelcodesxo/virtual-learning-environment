...
Exam generation is optional and disabled by default. It uses OpenRouter-compatible models to select a chapter from a description, map its topics, and generate the final exam, so enabling it requires `EXAM_BUILDER_ENABLED=true` and an `OPENROUTER_API_KEY`. By default, topic mapping uses the same OpenRouter model as exam writing (`EXAM_MODEL`). To use Ollama for topic mapping, set `EXAM_TOPIC_MAP_PROVIDER=ollama`; when `EXAM_TOPIC_MAP_MODEL` is not set, it uses `LLM_MODEL`. Setting `EXAM_TOPIC_MAP_MODEL` explicitly overrides the selected provider's default model. Model requests time out after five minutes by default; set `LLM_REQUEST_TIMEOUT_SECONDS` to adjust that limit.

Before generating an exam, choose one or more Bloom's taxonomy levels: remember, understand, apply, analyze, evaluate, or create. Both topic mapping and question writing receive every ordered excerpt from the full selected document, chapter, or section. The question-writing model also receives the topic map and selected levels. A small question count cannot guarantee a question on every topic, even though the models receive the full source; they prioritize central ideas and any requested focus. If a model reports that the source exceeds its context window, generation fails without saving an exam; choose a model with a larger context window or a shorter selected source.
